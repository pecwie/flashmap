...
class Map(ipyleaflet.Map):
    ...
    def add_basemap(self, basemap="OpenTopoMap"):
        ...
        if basemap not in allowed_basemaps:
            print(
                f"Invalid basemap name: {basemap}. Choose from {list(allowed_basemaps.keys())}"
            )
        else:
    ...
    def add_geojson(self, url, zoom_to_layer=True, hover_style=None, **kwargs):
        ...
        data_gj = ipyleaflet.GeoJSON(
            data=data.__geo_interface__, hover_style=hover_style
    ...
    if zoom_to_layer:
        ...
        self.fit_bounds(
            [[data_bounds[1], data_bounds[0]], [data_bounds[3], data_bounds[2]]]
        )  # fitting map layout to the bounding box
    ...
    def add_shp(self, data, **kwargs):
        ...
        data_gj = ipyleaflet.GeoJSON(
            data=data_gdf.__geo_interface__
        )  # converting GeoDataFrame to GeoJSON
        self.add_layer(data_gj)
...
class FoliumMap:
    ...
    def add_basemap(self, basemap="OpenStreetMap"):
        ...
        if basemap not in basemaps:
            print(
                f"Invalid basemap name: {basemap}. Choose from {list(basemaps.keys())}"
            )
        else:
            folium.TileLayer(
                tiles=basemaps[basemap], name=basemap, attr=basemap
            ).add_to(self.map)
